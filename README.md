...
| `EXT_DEV`        | `"true"` for dev extensions loaded from `dev_paths`; `"false"` for installed extensions                                                                                                                                                            |
| `EXT_SERVER_URL` | Tabibu server URL — used only by `sdk.HTTPClient()`                                                                                                                                                                                                |
| `EXT_JWT_SECRET` | Ephemeral HS256 secret generated per spawn for validating WebView JWTs via `sdk.ValidateToken()`. Always generated by Tabibu — setting this in `.env` has no effect when Tabibu is the parent.                                                     |
| `EXT_API_KEY`    | Per-spawn 64-char hex key used by the SDK to exchange a server JWT at startup. Generated by Tabibu, revoked on process exit, never written to disk. Setting this in `.env` is only meaningful for standalone testing without a live Tabibu server. |
